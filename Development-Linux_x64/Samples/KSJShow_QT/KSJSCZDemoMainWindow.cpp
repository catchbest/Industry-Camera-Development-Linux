...
void CKSJSCZDemoMainWindow::UpdateDeviceInfo()
{
	...
	bool bAwaibaUsed;
	KSJ_AWAIBA_IsUsed(m_nCamareIndex, &bAwaibaUsed);
	if (bAwaibaUsed)
	{
		ui->SegmentSpinBox->blockSignals(true);
        ui->BlackLevelSpinBox->blockSignals(true);

		ui->groupBox_2->setTitle("Line Scan Sensor Gain");
		ui->label_19->setText("Seg0:");
		ui->label_13->setText("Seg1:");
		ui->label_16->setText("Seg2:");

		int nSegmentNum;
		KSJ_AWAIBA_GetSegmentNum(m_nCamareIndex, &nSegmentNum);

		if (nSegmentNum == 1)
		{
			ui->SegmentSpinBox->setRange(0, 0);

			ui->label_19->setVisible(true);
			ui->label_13->setVisible(false);
			ui->label_16->setVisible(false);

            ui->GainRedSpinBox->setVisible(true);
            ui->GainGreenSpinBox->setVisible(false);
            ui->GainBlueSpinBox->setVisible(false);
		}
		else if (nSegmentNum == 2)
		{
			ui->SegmentSpinBox->setRange(0, 1);
            ...
            ui->GainRedSpinBox->setVisible(true);
            ui->GainGreenSpinBox->setVisible(true);
            ui->GainBlueSpinBox->setVisible(false);
		}
		else if (nSegmentNum >= 3)
		{
			ui->SegmentSpinBox->setRange(0, 2);
            ...
            ui->GainRedSpinBox->setVisible(true);
            ui->GainGreenSpinBox->setVisible(true);
            ui->GainBlueSpinBox->setVisible(true);
		}
		
		ui->SegmentSpinBox->setValue(0);

		int nBlackLevel = 0;

		if (nSegmentNum == 1)
		{
			KSJ_GetParam(m_nCamareIndex, KSJ_BLACKLEVEL, &nBlackLevel);
		}
		else
		{
			KSJ_AWAIBA_GetBlackLevel(m_nCamareIndex, 0, &nBlackLevel);
		}

		ui->BlackLevelSpinBox->setValue(nBlackLevel);

		ui->SegmentSpinBox->blockSignals(false);
		ui->BlackLevelSpinBox->blockSignals(false);
	}
	else
    {
		ui->groupBox_2->setTitle("Gain");
		ui->label_19->setText("Red:");
		ui->label_13->setText("Green:");
		ui->label_16->setText("Blue:");
		ui->label_19->setVisible(true);
		ui->label_13->setVisible(true);
		ui->label_16->setVisible(true);
...
void CKSJSCZDemoMainWindow::on_SegmentSpinBox_valueChanged(int value)
{
	if (m_nCamareIndex == -1)    return;

	int nSegment = ui->SegmentSpinBox->value();
	...
	{
		KSJ_AWAIBA_GetBlackLevel(m_nCamareIndex, nSegment, &nBlackLevel);
	}
...
void CKSJSCZDemoMainWindow::on_BlackLevelSpinBox_valueChanged(int value)
{
	if (m_nCamareIndex == -1)    return;

	int nSegIndex = ui->SegmentSpinBox->value();
	...
	if (nSegmentNum == 1)
	{
		KSJ_SetParam(m_nCamareIndex, KSJ_BLACKLEVEL, value);
	}
	else
	{
		KSJ_AWAIBA_SetBlackLevel(m_nCamareIndex, nSegIndex, value);
	}
}

void CKSJSCZDemoMainWindow::on_AutoBlackLevelPushButton_clicked()
{
	if (m_nCamareIndex == -1)    return;
	...
	KSJ_AWAIBA_AutoBlackLevel(m_nCamareIndex, nSegIndex);
	...
	{
		KSJ_AWAIBA_GetBlackLevel(m_nCamareIndex, nSegIndex, &nBlackLevel);
	}
